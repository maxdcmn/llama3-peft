...
def normalize_finetome(ex):
    return {
        "conversations": [
            {"role": ROLE_MAP.get(msg.get("from"), msg.get("from", "user")), "content": msg.get("value", "")}
...
def main():
    ...
    base = [normalize_finetome(ex) for ex in load_dataset("mlabonne/FineTome-100k", split="train")]
    ...
    print(f"✓ Pushed {len(base)} FineTome + {len(ddg)} DDG tool-calling examples to {repo_id}")
